...
public class BroadcastUserListService implements IUserListService {
  ...
  public void sendOnlineMessage() {
    try {
      udp.broadcastMessage(
          new Gson().toJson(new UserInfo(Constants.HOSTNAME, UserInfo.statusBroadCast)));
    } catch (IOException e) {
      e.printStackTrace();
    }
  }
  ...
  public void sendRegisterMessage(InetAddress destAddress) {
    try {
      udp.sendMessage(new Gson().toJson(new UserInfo(Constants.HOSTNAME, UserInfo.statusRegister)),
          destAddress);
    } catch (IOException e) {
      e.printStackTrace();
    }
